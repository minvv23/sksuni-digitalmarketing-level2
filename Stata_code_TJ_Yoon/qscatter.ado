*! version 1.0, March 3, 2015 Florian Zettelmeyer
*! based on binscatter 
*! version 7.02  24nov2013  Michael Stepner, stepner@mit.edu
*! The only changes are that the default is no line and that I draw vertical lines between the bins.
*! Also, i have changed the xq and genxq to qvar and genqvar.
...
program define qscatter, eclass sortpreserve
	version 12.1
	
	syntax varlist(min=2 numeric) [if] [in] [aweight fweight], [by(varname) ///
		Nquantiles(integer 20) GENqvar(name) discrete qvar(varname numeric) MEDians ///
		...
		/* LEGACY OPTIONS */ nbins(integer 20) create_qvar x_q(varname numeric) symbols(string) method(string) unique(string) ///
		*]
	...
	* Misc checks
	if ("`genqvar'"!="" & ("`qvar'"!="" | "`discrete'"!="")) | ("`qvar'"!="" & "`discrete'"!="") {
		di as error "Cannot specify more than one of genqvar(), qvar(), and discrete simultaneously."
		exit
	}
	if ("`genqvar'"!="") confirm new variable `genqvar'
	if ("`qvar'"!="") {
		capture assert `qvar'==int(`qvar') & `qvar'>0
		if _rc!=0 {
			di as error "qvar() must contain only positive integers."
			exit
		}
		
		if ("`controls'`absorb'"!="") di as text "warning: qvar() is specified in combination with controls() or absorb(). note that binning takes places after residualization, so the qvar variable should contain bins of the residuals."
	}
	if `nquantiles'!=20 & ("`qvar'"!="" | "`discrete'"!="") {
		di as error "Cannot specify nquantiles in combination with discrete or an qvar variable."
		exit
	}
	...
	* Mark sample (reflects the if/in conditions, and includes only nonmissing observations)
	marksample touse
	markout `touse' `by' `qvar' `controls' `absorb', strok
	qui count if `touse'
	...
	* Specify and/or create the qvar var, as necessary
	if "`qvar'"=="" {

		if !(`touse_first'==1 & word("`:sortedby'",1)=="`x_r'") sort `touse' `x_r'
	
		if "`discrete'"=="" { /* qvar() and discrete are not specified */
			...
			* Check whether the number of unique values > nquantiles, or <= nquantiles
			...
			if (_rc==0) { /* number of unique values <= nquantiles, set to discrete */
				...
				if ("`genqvar'"!="") di as text `"note: the x-variable has fewer unique values than the number of bins specified (`nquantiles').  It will therefore be treated as discrete, and genqvar() will be ignored"'

				local qvar `x_r'
				local nquantiles=r(r)
				if ("`by'"=="") {
					tempname qvar_boundaries qvar_values
					matrix `qvar_boundaries'=r(boundaries)		
					matrix `qvar_values'=r(values)
				}
			}
			else if (_rc==134) { /* number of unique values > nquantiles, perform binning */
				if ("`genqvar'"!="") local qvar `genqvar'
				else tempvar qvar
	
				if ("`fastxtile'"!="nofastxtile") fastxtile `qvar' = `x_r' `wt' in `touse_first'/`touse_last', nq(`nquantiles') randvar(`randvar') randcut(`randcut') randn(`randn')
				else xtile `qvar' = `x_r' `wt' in `touse_first'/`touse_last', nq(`nquantiles')

				if ("`by'"=="") {
					mata: characterize_unique_vals_sorted("`qvar'",`touse_first',`touse_last',`nquantiles')
		...
		else { /* discrete is specified, qvar() & genqvar() are not */
		...
		if !(`touse_first'==1 & word("`:sortedby'",1)=="`qvar'") sort `touse' `qvar'
		
		* set nquantiles & boundaries
		mata: characterize_unique_vals_sorted("`qvar'",`touse_first',`touse_last',`=`samplesize'/2')
	...
	********** Compute scatter points **********
	...
	forvalues b=1/`bynum' {
		if ("`by'"!="") {
			mata: characterize_unique_vals_sorted("`qvar'",`=`by_boundaries'[`b',1]',`=`by_boundaries'[`b',2]',`nquantiles')
			tempname qvar_boundaries qvar_values
			matrix `qvar_boundaries'=r(boundaries)
			matrix `qvar_values'=r(values)
		}
		/* otherwise qvar_boundaries and qvar_values are defined above in the binning code block */

		* Define x-means
		tempname xbin_means xbin_cuts
		if ("`discrete'"=="discrete") {
			means_in_boundaries `x_r' `wt', bounds(`qvar_boundaries') `medians'
			matrix `xbin_cuts'=r(cuts)
			matrix `xbin_means'=`qvar_values'
		}
		else {
			means_in_boundaries `x_r' `wt', bounds(`qvar_boundaries') `medians'
		...
		* LOOP over y-vars to define y-means
		...
		foreach depvar of varlist `y_vars_r' {
			...
			means_in_boundaries `depvar' `wt', bounds(`qvar_boundaries') `medians'
		...
		* LOOP over y-vars
		...
		foreach depvar of varlist `y_vars' {
			...
			* LOOP over rows (each row contains a coordinate pair)
			...
			local row2=`row'
			local xdivval=`xbin_cuts'[`row2',1]
			
			while (`xdivval'!=.) {
				local xdivlines `xdivlines' `xdivval'
				local ++row2	
				local xdivval=`xbin_cuts'[`row2',1]
			}

			local xline10="xline(`xdivlines', lpattern(dash) lcolor(gs13) lwidth(vthin))"
	...
	* Display graph
	local graphcmd twoway `scatters' `fits', graphregion(fcolor(white)) `xlines' xtitle(`x_var') ytitle(`ytitle') legend(`legend_labels' order(`order')) `options' `xline10'
...
program define means_in_boundaries, rclass
	...
	* Create convenient weight local
	...
	if ("`medians'"!="medians") {
		forvalues i=1/`r' {
			qui sum `varlist' in `=`boundsmat'[`i',1]'/`=`boundsmat'[`i',2]' `wt'
			matrix means[`i',1]=r(mean)
			matrix temp[`i',1]=r(min)
			matrix temp[`i'+1,2]=r(max)
			if `i'>1 {
				matrix cuts[`i'-1,1]=(temp[`i',1]+temp[`i',2])/2
				}
		}
	}
	else {
