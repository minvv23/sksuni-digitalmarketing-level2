{smcl}
{* *! version 1.0  3mar2015}{...}
{viewerjumpto "Syntax" "qscatter##syntax"}{...}
{viewerjumpto "Description" "qscatter##description"}{...}
{viewerjumpto "Options" "qscatter##options"}{...}
{viewerjumpto "Examples" "qscatter##examples"}{...}
{viewerjumpto "Saved results" "qscatter##saved_results"}{...}
{viewerjumpto "Author" "qscatter##author"}{...}
{viewerjumpto "Acknowledgements" "qscatter##acknowledgements"}{...}
{title:Title}

{p2colset 5 19 21 2}{...}
{p2col :{hi:qscatter} {hline 2}}Quantile (binned) scatterplots{p_end}
{p2colreset}{...}
...
{synoptline}
{syntab :Main}
{synopt :{opth by(varname)}}plot separate series for each group (see {help qscatter##by_notes:important notes below}){p_end}
...
{synopt :{opth gen:qvar(varname)}}generate quantile variable containing the bins{p_end}
{synopt :{opt discrete}}each x-value to be used as a separate bin{p_end}
{synopt :{opth qvar(varname)}}variable which already contains bins; bins therefore not recomputed{p_end}
...
{syntab :Fit Line}
{synopt :{opth line:type(qscatter##linetype:linetype)}}type of fit line; default is {bf:lfit}, may also be {bf:qfit}, {bf:connect}, or {bf:none}{p_end}
...
{pstd}
{opt qscatter} generates quantile (binned) scatterplots, and is optimized for speed in large datasets.

{pstd}
Quantile (binned) scatterplots provide a non-parametric way of visualizing the relationship between two variables.
With a large number of observations, a scatterplot that plots every data point would become too crowded
to interpret visually.  {cmd:qscatter} groups the x-axis variable into equal-sized bins, computes the
...
{pstd}
{opt qscatter} provides built-in options to control for covariates before plotting the relationship
(see {help qscatter##controls:Controls}).  Additionally, {cmd:qscatter} will plot fit lines based
on the underlying data, and can automatically handle regression discontinuities (see {help qscatter##fit_line:Fit Line}).
...
{pmore}Users should be aware of the two ways in which {cmd:qscatter} does not condition on by-values:
...
covariates using a different model, you can residualize your x- and y-variables beforehand using your desired model then run {cmd:qscatter}
on the residuals you constructed.

{phang3}2) When not combined with {opt discrete} or {opt qvar()}, the program constructs a single set of bins
...
by-value), you can construct a variable containing your desired bins beforehand, then run {cmd:qscatter} with {opt qvar()}.
...
This option cannot be combined with {opt discrete} or {opt qvar()}.

{pmore}
Quantiles are created after residualization when combined with {opt controls()} or {opt absorb()}.
...
{phang}{opth gen:qvar(varname)} creates a categorical variable containing the computed bins.
This option cannot be combined with {opt discrete} or {opt qvar()}.

{phang}{opt discrete} specifies that the x-variable is discrete and that each x-value is to be treated as
a separate bin. {cmd:qscatter} will therefore plot the mean y-value associated with each x-value.
This option cannot be combined with {opt nquantiles()}, {opt genqvar()} or {opt qvar()}.
...
{phang}{opth qvar(varname)} specifies a categorical variable that contains the bins to be used, instead of {cmd:qscatter} generating them.
This option is typically used to avoid recomputing the bins needlessly when {cmd:qscatter} is being run repeatedly on the same sample
and with the same x-variable.
It may be convenient to use {opt genqvar(binvar)} in the first iteration, and specify {opt qvar(binvar)} in subsequent iterations.
...
This option cannot be combined with {opt nquantiles()}, {opt genqvar()} or {opt discrete}.

{pmore}
Care should be taken when combining {opt qvar()} with {opt controls()} or {opt absorb()}.  Binning takes place after residualization,
...
To do so, {cmd:qscatter} runs a regression of each variable on the controls, generates the residuals, and adds the sample mean of
...
To do so, {cmd:qscatter} runs an {helpb areg} of each variable with {it:absorb(varname)} and any {opt controls()} specified.  It then generates the
...
{marker linetype}{...}
{phang}{opth line:type(qscatter##linetype:linetype)} specifies the type of line plotted on each series.
...
{pmore}Any unrecognized options added to {cmd:qscatter} are appended to the end of the twoway command which generates the
binned scatter plot.
...
{pmore}{cmd:fastxtile} is built into the {cmd:qscatter} code, but may also be installed
separately from SSC ({stata ssc install fastxtile:click here to install}) for use outside of {cmd:qscatter}.
...
{phang2}. {stata qscatter wage tenure}{p_end}

{pstd}The scatter was too crowded to be easily interpetable. The qscatter is cleaner, but a linear fit looks unreasonable.{p_end}

{pstd}Try a quadratic fit.{p_end}
{phang2}. {stata qscatter wage tenure, line(qfit)}{p_end}

{pstd}We can also plot a linear regression discontinuity.{p_end}
{phang2}. {stata qscatter wage tenure, rd(2.5)}{p_end}

{pstd} What is the relationship between age and wages?{p_end}
{phang2}. {stata scatter wage age}{p_end}
{phang2}. {stata qscatter wage age}{p_end}

{pstd} The qscatter is again much easier to interpret. (Note that {cmd:qscatter} automatically
...
{phang2}. {stata qscatter wage age, by(race)}{p_end}
...
{phang2}. {stata qscatter wage age, by(race) absorb(occupation)}{p_end}

{pstd} A very different picture emerges.  Let's label this graph nicely.{p_end}
{phang2}. {stata qscatter wage age, by(race) absorb(occupation) msymbols(O T) xtitle(Age) ytitle(Hourly Wage) legend(lab(1 White) lab(2 Black))}{p_end}
...
{pstd}
{cmd:qscatter} saves the following in {cmd:e()}:
...
{pstd}Florian Zettelmeyer{p_end}

{marker acknowledgements}{...}
{title:Acknowledgements}

{pstd}The present version of {cmd:qscatter} is a minor customization of {cmd:binscatter} written by Michael Stepner, stepner@mit.edu.{p_end}

{pstd}The modifications are:{p_end}

{pstd}(1) The default is to draw no line.{p_end}
{pstd}(2) The graph contains vertical lines between the bins.{p_end}
{pstd}(3) The {opt qvar(binvar)} is renamed to {opt qvar(binvar)} {p_end}
{pstd}(4) The {opt genqvar(binvar)} is renamed to {opt genqvar(binvar)}{p_end}

{pstd}{cmd:binscatter} contains the following acknowledgements:
{pstd} The present version of {cmd:qscatter} is based on a program first written by Jessica Laird. This program was developed under the guidance and direction of Raj Chetty and John
Friedman.  Laszlo Sandor provided suggestions which improved the program considerably, and offered abundant help
testing it.
Thank you also to the users of early versions of the program who devoted time to reporting
